...
public class PointSET {
    ...
    public Iterable<Point2D> range(RectHV rect) {
        ...
        return new IntersectionPoints(rect);
    ...
    public Point2D nearest(Point2D p) {
        ...
        for (Point2D point2D : pointSet) {
            if (p.distanceTo(point2D) < nearestDistance) {
                nearestNeighbour = point2D;
                nearestDistance = p.distanceTo(point2D);
            }
        }
    ...
    private class IntersectionPoints implements Iterable<Point2D> {
        private Bag<Point2D> point2DBag = new Bag<>();

        public IntersectionPoints(RectHV rect) {
            for (Point2D point2D : pointSet) {
